/**
 * Pure POS sale validation — kept out of the route so money/stock rules
 * can be unit-tested without Next.js or a live database.
 */
...
/** Simulate two cashiers racing for the last N units (pure FEFO math). */
export function concurrentLastUnitOutcome(params: {
  available: number
  requestA: number
  requestB: number
}): { aFulfilled: number; bFulfilled: number; leftover: number } {
  let stock = params.available
  const take = (n: number) => {
    const got = Math.min(Math.max(0, n), stock)
    stock -= got
    return got
  }
  // Deterministic race: A commits first (DB transaction winner).
  const aFulfilled = take(params.requestA)
  const bFulfilled = take(params.requestB)
  return { aFulfilled, bFulfilled, leftover: stock }
}
