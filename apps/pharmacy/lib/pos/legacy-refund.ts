...
export function planLegacyOrderRefund(
  ...
  const items = requested && requested.length > 0 ? requested : lines.map((line) => ({
    id: line.id,
...
export const LEGACY_ORDER_REFUND_WARNING =
  "Legacy order refund recorded financially only. It did not restore sellable stock or create a batch. Receive or quarantine returned units through inventory."
