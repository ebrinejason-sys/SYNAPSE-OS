...
describe("legacy order refund stock authority", () => {
  ...
  it("documents the non-sellable outcome", () => {
    expect(LEGACY_ORDER_REFUND_WARNING.includes("did not restore sellable stock")).toBe(true)
    expect(LEGACY_ORDER_REFUND_WARNING.toLowerCase().includes("restored product quantity")).toBe(false)
  })
})
