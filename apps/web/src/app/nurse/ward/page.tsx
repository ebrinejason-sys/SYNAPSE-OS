    ...
    <main className="min-h-screen bg-base text-primary-color p-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-8">
          <h2 className="text-sm font-semibold uppercase text-muted-color">Occupied beds</h2>
          <ul className="mt-3 space-y-2">
            {occupiedBeds.map((bed) => (
              <li key={bed.bedId} className="rounded-xl border border-subtle bg-surface p-4">
                <p className="font-medium">
                  {bed.ward} · Bed {bed.bedNumber} {bed.room ? `· ${bed.room}` : ''}
                </p>
                <p className="text-sm text-secondary-color">{bed.patient.fullName}</p>
                <p className="text-xs text-muted-color">{bed.patient.mrn ?? bed.patient.id.slice(0, 8)}</p>
              </li>
            ))}
            {occupiedBeds.length === 0 && !error ? (
              <p className="text-sm text-muted-color">No occupied beds.</p>
            ) : null}
          </ul>
        </section>
        ...
        <section className="mt-8">
          ...
          <ul className="mt-3 space-y-2">
            {openTasks.map((task) => (
              <li key={task.id} className="rounded-xl border border-subtle bg-surface p-4 text-sm">
            ...
            {openTasks.length === 0 && !error ? (
              <p className="text-sm text-muted-color">No open inpatient tasks.</p>
            ) : null}
          </ul>
        </section>
