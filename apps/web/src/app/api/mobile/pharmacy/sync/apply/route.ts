...
export async function POST(request: NextRequest) {
  ...
  if (!saleResponse.ok) {
    ...
    const insufficientStock =
      code === 'INSUFFICIENT_STOCK' || code === 'EXPIRED_BATCH_BLOCKED'
    const deterministic =
      insufficientStock ||
      (saleResponse.status >= 400 &&
        saleResponse.status < 500 &&
        ![401, 402, 403, 408, 429].includes(saleResponse.status))
    const nextStatus = deterministic ? 'rejected' : 'queued'
    ...
    return NextResponse.json(
      {
        ...saleResult,
        outcome: deterministic ? 'rejected' : 'retry',
        reason,
      },
