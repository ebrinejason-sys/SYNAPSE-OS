...
import { Zap, Plus, Trash2, ShieldAlert, BookOpen } from 'lucide-react'
    ...
    try {
      ...
      if (!res.ok) {
        setResult({
          ...
          interactions: [],
          advisory: [],
          summary: data.error ?? 'Unable to check interactions.',
        })
      } else {
        setResult(data)
      }
      ...
      setResult({
        ...
        interactions: [],
        advisory: [],
        summary: 'Unable to check interactions. Verify manually before dispensing.',
      })
    }
    setChecking(false)
  }

  const bannerTone = !result
    ? null
    : !result.available
      ? 'warn'
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
          Deterministic curated pack first; AI is advisory only and never clears a sale as safe.
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div>
              ...
              >
                {!result.available
                  ? 'Check unavailable — not cleared'
                  : result.interactions.length > 0
                    ? 'Interactions found — pharmacist review required'
                    : 'Advisory only — not a safety clearance'}
              </p>
              ...
              {result.explanation ? (
                ...
                >
                  <p className="font-semibold mb-1" style={{ color: 'var(--brand-orange)' }}>
                    Counselling narration (pack hits only)
                  </p>
                  {result.explanation}
                </div>
              ) : null}
          ...
          {[...result.interactions, ...(result.advisory ?? [])].map((interaction, i) => (
            <div
              key={i}
            ...
            >
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {interaction.source?.includes('Generative') ? (
                  <span
              ...
              >
                <p className="text-xs font-semibold" style={{ color: 'var(--brand-orange)' }}>
                  Recommendation:
                </p>
