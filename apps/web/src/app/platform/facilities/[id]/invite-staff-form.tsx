...
export function InviteStaffForm({ tenantId, departments }: { tenantId: string; departments: Department[] }) {
  ...
  return <form action={submit} className="mt-4 grid gap-3 border-t border-slate-800 pt-4 sm:grid-cols-2">
    ...
    <label className="text-sm text-slate-400">Role<select name="role" defaultValue="lab_tech" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-slate-100"><option value="facility_admin">Laboratory Administrator</option><option value="lab_tech">Lab Technician</option><option value="lab_scientist">Lab Scientist / Verifier</option><option value="billing_officer">Cashier</option><option value="quality_officer">Quality Officer</option><option value="instrument_manager">Instrument Manager</option></select></label>
