...
test.describe("Demo Golden Journey", () => {
  test("land → reception → nurse → doctor → lab → review → pharmacy → billing → timeline/network", async ({ page }) => {
    ...
    await expect(doctorCopilot.getByText(/Requires qualified human review/i)).toBeVisible()
    ...
    await expect(labCopilot.getByText(/Requires qualified human review/i)).toBeVisible()
