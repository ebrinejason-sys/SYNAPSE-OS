...
test.describe("Demo Intelligence isolation", () => {
  test("playground uses synthetic Amina, persists decisions, and never mutates production healthcare", async ({ page }) => {
    ...
    await expect(copilot.getByText(/Requires qualified human review/i).first()).toBeVisible()
    await expect(copilot.getByText(/synthetic-fallback/i).first()).toBeVisible()
