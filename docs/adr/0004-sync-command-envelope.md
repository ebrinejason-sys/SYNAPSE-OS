...
3. Same `commandId` + same `payloadHash` is a safe replay. Same `commandId` + different hash is human-review conflict.
4. Persistence runtime (Expo SQLite / web encrypted store / Edge) is owned by the Offline Sync agent. The contract is owned by Core.
5. Pharmacy POS, inventory RPCs, and `complete_pharmacy_sale` remain the online execution engines. Sync applies those engines; it does not replace them.
