...
Status: **accepted for Milestone A**. Core/Grid owns these types. Feature agents consume them.
...
| Encounter | `@synapse/interop` `clinical-contracts` | Not an operational UI |
...
| IndicatorDefinition | `@synapse/interop` `clinical-contracts` | Public health later |
...
UI copy must match `docs/implementation/capability-registry.json`. If status is `PLANNED`, do not ship an operational screen for it.
