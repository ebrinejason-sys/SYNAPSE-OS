...
- Online POS is operational via `complete_pharmacy_sale` (FEFO, idempotent).
...
- `offline_mutation_outbox` exists in SQL; no application writer yet.
...
7. Automated tests cover: happy path, restart, duplicate replay, insufficient stock, tenant isolation.
...
- Clinical journey / FHIR / ICD-11.

## Pharmacy domain remaining (online)

Transfer **execution** (not draft-only rows), counts, recalls, quarantine UX — proposed SQL reviewed by Core. Do not block the offline slice on full warehouse features, but do not claim them OPERATIONAL.
