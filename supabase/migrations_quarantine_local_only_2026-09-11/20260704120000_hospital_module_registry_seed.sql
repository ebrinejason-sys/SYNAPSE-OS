...
-- ── Proof gate queries (run manually after apply) ─────────────────────────────
-- SELECT key, jsonb_array_length(value) FROM platform_billing_config WHERE key = 'hospital_module_registry';
-- SELECT module, resource, action FROM capabilities WHERE module IN ('opd','ipd','ward','lab','radiology','maternity','theatre','mortuary','billing','migrate') ORDER BY 1,2,3;
-- SELECT p.slug, COUNT(pf.id) FROM subscription_plans p LEFT JOIN plan_features pf ON pf.plan_id = p.id WHERE p.facility_type = 'hospital' GROUP BY p.slug;
-- SELECT COUNT(*) FROM hospital_modules WHERE is_active = true;
