-- Claim POS idempotency keys inside complete_pharmacy_sale so retries cannot double-sell.
-- Optional p_idempotency_key: when set, claim-or-return happens in the same transaction as the sale.
