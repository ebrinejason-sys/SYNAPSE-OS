-- FIX: pharmacy_pos_sale_items.line_total is a GENERATED column
--   (quantity*unit_price - discount_amount). complete_pharmacy_sale() inserted
--   an explicit value into it, so the RPC failed on EVERY call with
--   "cannot insert a non-DEFAULT value into column line_total" — the sale path
--   had never once completed (pharmacy_pos_sales = 0 rows, discovered during
--   the Workstream G6 sale-path smoke on 2026-07-15).
--
-- The manual expression the RPC computed is identical to the generated column
-- definition, so the fix is simply to omit line_total from the INSERT and let
-- Postgres compute it. No behavior change: FEFO batch selection, Kampala-time
-- receipt numbering, and confirmed_by/confirmed_at stamping are unchanged.
--
-- Verified live: a 3-unit sale produced receipt R-20260715-0001, line_total
-- 15000, batch decremented 100 -> 97. Applied to project qfqakzmjatszisuqjwon
-- via apply_migration; this file tracks the same DDL in repo history.

CREATE OR REPLACE FUNCTION public.complete_pharmacy_sale(
  p_tenant_id uuid, p_cashier_id uuid, p_items jsonb, p_payment_method text,
  p_session_id uuid DEFAULT NULL::uuid, p_cart_id uuid DEFAULT NULL::uuid,
  p_payment_ref text DEFAULT NULL::text, p_discount_total numeric DEFAULT 0,
  p_tax_amount numeric DEFAULT 0, p_patient_id uuid DEFAULT NULL::uuid,
...
begin
  ...
  values
    (v_sale_id, p_tenant_id, p_session_id, p_cart_id, p_cashier_id, p_patient_id, v_receipt,
     ...
     p_payment_method, p_payment_ref, 'completed', coalesce(p_confirmed_by, p_cashier_id), now());
  ...
  set subtotal = v_subtotal, discount_total = v_discount, total_amount = v_total, updated_at = now()
  where id = v_sale_id;
