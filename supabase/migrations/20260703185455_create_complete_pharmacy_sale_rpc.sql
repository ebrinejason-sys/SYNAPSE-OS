...
-- Remote version: 20260703185455  name: create_complete_pharmacy_sale_rpc
-- DO NOT edit to "fix" history; additive follow-ups belong in new migrations.


-- Atomic POS checkout: FEFO batch consumption, stock decrement, sale + items + receipt in ONE transaction.
-- Service-role only (custom auth calls this from Next.js API routes).

create or replace function public.complete_pharmacy_sale(
  p_tenant_id uuid,
  p_cashier_id uuid,
  p_items jsonb,                      -- [{"product_id": uuid, "quantity": int, "unit_price"?: numeric, "discount_amount"?: numeric, "batch_id"?: uuid}]
  p_payment_method text,
  p_session_id uuid default null,
  p_cart_id uuid default null,
  p_payment_ref text default null,
  p_discount_total numeric default 0, -- sale-level discount, added to line discounts
  p_tax_amount numeric default 0,
  p_patient_id uuid default null,
  p_confirmed_by uuid default null    -- defaults to cashier
) returns jsonb
language plpgsql
security definer
    ...
    loop
      ...
      insert into pharmacy_pos_sale_items
        (id, sale_id, tenant_id, product_id, batch_id, quantity, unit_price,
         discount_amount, line_total, stock_decremented)
      values
        (gen_random_uuid(), v_sale_id, p_tenant_id, v_product.id, v_batch.id, v_take, v_unit_price,
         ...
         (v_take * v_unit_price) - (case when not v_disc_applied then v_line_discount else 0 end),
         true);
...
revoke all on function public.complete_pharmacy_sale(uuid, uuid, jsonb, text, uuid, uuid, text, numeric, numeric, uuid, uuid) from public;
grant execute on function public.complete_pharmacy_sale(uuid, uuid, jsonb, text, uuid, uuid, text, numeric, numeric, uuid, uuid) to service_role;

create index if not exists idx_pos_sales_tenant_created on public.pharmacy_pos_sales (tenant_id, created_at desc);
create index if not exists idx_batches_fefo on public.pharmacy_product_batches (product_id, expiry_date) where is_active and quantity > 0;
