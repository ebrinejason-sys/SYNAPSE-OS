...
--   * lab_results / encounter_diagnoses / insurance_claims already exist and
--     are not touched.
...
-- ============================================================
-- DRUG INVENTORY (hospital formulary — matches live inventory UI)
-- ============================================================
...
-- Tenant isolation (server routes use the service role and bypass RLS;
-- these policies only matter for direct PostgREST access).
...
-- Score definitions: public catalog, read-only
CREATE POLICY "Score definitions public read" ON score_definitions
  FOR SELECT USING (is_active = true);

-- Outbreak alerts: public read of active alerts, platform admin manages
...
-- Facility referrals: both sides can see, sender inserts, either side updates
...
-- Cross tenant access grants: platform admin only via PostgREST
CREATE POLICY "Platform admin manage cross tenant access" ON cross_tenant_access
  FOR ALL USING (is_platform_admin());

-- Offline sync queue: own rows only
CREATE POLICY "Own sync queue" ON offline_sync_queue
  FOR ALL USING (tenant_id = current_tenant_id() AND user_id = auth.uid());

-- imid_codes, imid_access_log, pilot_applications: RLS enabled with no
-- public policies — service-role only (edge functions / server routes).

-- ============================================================
-- REALTIME (new tables the original baseline flagged for realtime)
-- ============================================================
DO $$
DECLARE
  t TEXT;
