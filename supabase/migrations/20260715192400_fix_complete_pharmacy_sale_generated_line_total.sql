...
-- Remote version: 20260715192400  name: fix_complete_pharmacy_sale_generated_line_total
-- DO NOT edit to "fix" history; additive follow-ups belong in new migrations.

-- FIX: pharmacy_pos_sale_items.line_total is a GENERATED column
--   (quantity*unit_price - discount_amount). The prior RPC body inserted an
--   explicit value into it, so complete_pharmacy_sale() failed on EVERY call
--   with "cannot insert a non-DEFAULT value into column line_total" — the sale
--   path had never actually completed (pharmacy_pos_sales = 0 rows).
-- The manual expression the RPC used is identical to the generated one, so the
-- fix is simply to drop line_total from the INSERT and let Postgres compute it.
-- No other behavior changes (FEFO, Kampala receipt numbering, confirmed_by/at).
