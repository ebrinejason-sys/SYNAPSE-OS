...
/**
 * Map a complete-sale HTTP failure onto outbox status.
 * 4xx domain errors (insufficient stock, validation) are rejected and must
 * never be retried as a new sale. Auth/rate-limit/network stay queued.
 */
export function classifyApplyHttpFailure(
  status: number,
  code?: string | null,
): "rejected" | "retry" {
  ...
  if (
    ...
    normalized === "EXPIRED_BATCH_BLOCKED" ||
    normalized === "EXPIRED_BATCH" ||
    normalized === "NO_SELLABLE_BATCHES"
  ) {
    return "rejected"
  }
  const retryableClient = status === 401 || status === 402 || status === 403 || status === 408 || status === 429
  if (status >= 400 && status < 500 && !retryableClient) return "rejected"
  return "retry"
}
