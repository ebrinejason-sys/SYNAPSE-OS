 ...
 *
 * Usage:
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/opd-dispense-live-journey.mjs
 *
...
function step(name, payload) {
  ...
  console.error(`[opd-dispense-live] ${name}`, payload.ok === false ? payload.error || payload : "ok")
}
...
try {
  ...
  const patientIns = await db.from("patients").insert({
    ...
    full_name: "Synthetic OPD Dispense Patient",
    sex: "F",
    is_deleted: false,
  })
  ...
  const encIns = await db.from("encounters").insert({
    ...
    chief_complaint: "fever — synthetic OPD dispense journey",
    clinical_stage: "GREEN",
    is_deleted: false,
    metadata: { is_synthetic: true, journey: "opd-dispense-live" },
  })
  ...
  const receive = await db.rpc("receive_pharmacy_stock", {
    ...
    p_supplier_ref: "opd-dispense-live-journey",
  })
  ...
  const outPath = join(outDir, `opd-dispense-live-${runId}.json`)
